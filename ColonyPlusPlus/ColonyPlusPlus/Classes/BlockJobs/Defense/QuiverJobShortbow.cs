...
{
    public class QuiverJobShortbow : BlockJobBase, IBlockJobBase, INPCTypeDefiner
    {
        ushort quiverType;
        Zombie target;

        public override string NPCTypeKey { get { return "cpp.shortbow"; } }
        ...
        public override void OnNPCDoJob(ref NPCBase.NPCState state)
        {
            ...
            if (target == null || !target.IsValid)
            {
                target = ZombieTracker.Find(position.Add(0, 1, 0), 10);
                if (target == null)
                {
        ...
        NPCTypeSettings INPCTypeDefiner.GetNPCTypeDefinition()
        {
            ...
            def.printName = "Shortbow guard";
            def.maskColor1 = new Color32(0, 255, 255, 255);
